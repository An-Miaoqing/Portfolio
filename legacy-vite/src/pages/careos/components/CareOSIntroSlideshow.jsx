...
const slides = [
  {
    ...
    message: 'One operational model connects customer demand, operations, workforce activity and finance.',
    points: [
  ...
  },
  {
    ...
    message: 'Architecture begins with the operating problem, its stakeholders and the rules governing real work.',
    points: [
  ...
  },
  {
    ...
    message: 'A customer need moves through coordinated operational stages until the work is commercially settled.',
    points: [
  ...
  },
  {
    ...
    message: 'Customers, operations and field employees use different interfaces governed by the same business rules.',
    points: [
  ...
  },
  {
    ...
    message: 'Business concepts become entities, relationships and constraints—not tables designed around screens.',
    points: [
  ...
  },
  {
    ...
    message: 'Technology supports the domain, architectural boundaries and need for maintainable production delivery.',
    points: [
...
function LifecycleVisual() {
  ...
  return (
    <div className="careos-slide-lifecycle" aria-label="Business lifecycle connected through the CareOS core" role="img">
      <div className="careos-slide-lifecycle__core">CareOS Core</div>
      <ol>
        {lifecycle.map(([stage, description], index) => (
          <li key={stage}>
            <span>0{index + 1}</span>
            <div><strong>{stage}</strong><small>{description}</small></div>
...
function ProblemVisual() {
  ...
  return (
    <div className="careos-slide-problem" aria-label="Fragmented operational inputs becoming one governed CareOS workflow" role="img">
      <div className="careos-slide-problem__fragments">
        {fragments.map((fragment) => <span key={fragment}>{fragment}</span>)}
      </div>
      <div className="careos-slide-problem__flow" aria-hidden="true"><i /><i /><i /><span>CONVERGE</span><i /><i /><i /></div>
      <strong>Connected Operational Platform<small>One source of operational truth</small></strong>
    </div>
  )
}

function BusinessAnalysisVisual() {
  const stages = ['Business Problem', 'Stakeholders', 'Pain Points', 'Business Rules', 'Process Analysis', 'Domain Model', 'System Design']

  return (
    <div className="careos-slide-analysis-process" aria-label="Business analysis process from stakeholders to implementation" role="img">
...
function WorkflowVisual() {
  const workflow = ['Customer Need', 'Request', 'Planning', 'Assignment', 'Service Delivery', 'Completion', 'Billing', 'Settlement']

  return (
    <div className="careos-slide-workflow careos-slide-workflow--lifecycle" aria-label="CareOS end-to-end business lifecycle" role="img">
  ...
  return (
    <div className="careos-slide-architecture" aria-label="CareOS layered system architecture" role="img">
      <div className="careos-slide-architecture__apps">
        <span>Customer Website<small>React + Vite</small></span>
        <span>Admin Application<small>React + Vite</small></span>
        <span>Employee App<small>Expo + React Native</small></span>
      </div>
      <i aria-hidden="true" />
      ...
      <div className="careos-slide-architecture__data"><span>Prisma</span><b>↓</b><span>PostgreSQL</span></div>
    </div>
  )
}
...
function DatabaseVisual() {
  ...
  return (
    <div className="careos-slide-database" aria-label="Database design process and future ER diagram placeholder" role="img">
      <div className="careos-slide-database__process">
        {stages.map((stage, index) => (
          <div key={stage}><span>0{index + 1}</span><strong>{stage}</strong>{index < stages.length - 1 && <i aria-hidden="true">↓</i>}</div>
        ))}
      </div>
...
function ApplicationMockup({ label, role, type }) {
  return (
    <div className={`careos-device-mockup careos-device-mockup--${type}`}>
      <div className="careos-device-mockup__chrome" aria-hidden="true"><i /><i /><i /></div>
      <MediaPlaceholder label={`${label} Screenshot`} format="slide" />
      <p><strong>{label}</strong><small>→ {role}</small></p>
    </div>
  )
}
...
function EngineeringVisual() {
  ...
  return (
    <div className="careos-slide-engineering" aria-label="CareOS engineering principles and supporting technologies" role="img">
      {principles.map(([principle, technologies], index) => (
        <div key={principle}><span>0{index + 1}</span><strong>{principle}</strong><small>{technologies}</small></div>
      ))}
    </div>
  )
}
...
export default function CareOSIntroSlideshow() {
  ...
  return (
    ...
    >
      ...
      <article className="careos-intro-slide" id="careos-intro-slide-panel" role="tabpanel" aria-live={isAutoplayPaused ? 'polite' : 'off'} aria-labelledby={`careos-slide-title-${activeSlide.id}`}>
        <div className="careos-intro-slide__copy">
          ...
          <ul>
            {activeSlide.points.map((point) => <li key={point}>{point}</li>)}
          </ul>
        </div>
      ...
      <div className="careos-intro-deck__controls">
        <div className="careos-intro-deck__arrows">
          <button type="button" onClick={showPrevious} aria-label="Show previous slide">←</button>
          <button type="button" onClick={showNext} aria-label="Show next slide">→</button>
        </div>
